...
export const selectAIRaider = (aiTeam: Player[]): Player | null => {
  ...
  return availableRaiders.sort((a, b) => (b.stats.agi + b.stats.str) - (a.stats.agi + a.stats.str))[0];
};
...
export const selectAITarget = (get: StoreGetter): Player | null => {
  ...
  const getProximityBonus = (defender: Player): number => {
    ...
    if (raiderLane === 'center' && defender.position.y >= 40 && defender.position.y <= 60) return 10;
 ...
 */
export const chooseAIAction = (get: StoreGetter): 'FEINT' | 'TOUCH' | 'BONUS' | 'RETREAT' => {
  const { 
    stamina, 
    raidTimer, 
    isDoOrDie, 
  ...
  // --- 3. Baulk Line Check ---
  if (!hasCrossedBaulkLine) {
    // Must cross the Baulk line.
    return (Math.random() < 0.7) ? 'TOUCH' : 'FEINT';
  }

  // --- 4. Bonus Point Check ---
  if (defenderCount >= 6 && pointsScoredThisRaid > 0) {
    // AI has scored and sees a bonus opportunity.
    if (Math.random() < 0.2) { // 20% chance to try for bonus
      return 'BONUS';
    }
  }
